package ksmart.tourproject.user.review.service;
...
import ksmart.tourproject.user.review.mapper.UReviewMapper;
import ksmart.tourproject.user.review.util.FileUtil;
...
public class UReviewService {
	...
	*/
	 ...
	 * 신고등록
	 */
	public void reviewReport(UReviewReport reviewReport) {
     
        String maxPrchsRevCd = uReviewMapper.getMaxReviewReportCd();
        String newPrchsRevCd;

        if (maxPrchsRevCd != null) {
            // "RTN_" 다음의 숫자 부분을 추출하여 정수로 변환하고 1을 더함
            int maxNumber = Integer.parseInt(maxPrchsRevCd.substring(4)) + 1;
            newPrchsRevCd = "RTN_" + maxNumber;
        } else {
        ...
        reviewReport.setReviewReportCode(newPrchsRevCd);
       
		uReviewMapper.reviewReport(reviewReport);
	}
